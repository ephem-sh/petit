---
title: writing documentation playbook
description: A full ruleset of how to write proper documentation for the "writer" skill
---
...
- **Examples:** Use meaningful names in examples; avoid placeholders like
  "foo" or "bar."
- **Terminology consistency:** Use the same terms the project uses.
  Read source code, config files, and existing docs to identify the
  project's vocabulary. Don't introduce synonyms or alternate names.

### Truth verification
Never fabricate output, behavior, or examples. Every claim must be
verifiable.

- **Terminal output:** Never make up what a command prints. Run the
  command first, or read the formatter source code. If you can't
  verify, mark the output as approximate.
- **CLI invocation:** Verify how the binary is actually invoked by
  end users. Check the `bin` field in `package.json` and whether
  the tool is installed globally or as a dependency. Show the
  invocation the user will actually type (for example, `npx tool`
  vs `tool`).
- **Package names:** Read the actual `package.json`, `Cargo.toml`,
  `pyproject.toml`, or `go.mod` for published names. These are the
  source of truth, not README files or project docs.
- **Code examples:** Must be complete and runnable. No pseudo-code
  in guides. If you can't verify an example runs, say so.
...
- **Naming:** Always refer to the project by its actual name as
  found in package.json or project config. Use lowercase, no article.
...
3.  **Link check:** Verify all new and existing links leading to or from modified
    ...
    updated.
4.  **Format:** Ensure consistent formatting across all modified files.
5.  **Behavioral verification:** If the project has runnable examples,
